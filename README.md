...
> [!WARNING]
> **不再建议登录使用。** 已有用户报告使用本工具的登录态 / token 下载能力可能触发小宇宙账号风控或封禁风险。请不要在重要账号上使用，也不要进行批量、高频或自动化下载。项目仅保留作研究与代码参考，使用者需自行承担账号风险。后续如果继续维护，方向会优先转向**免登录下载 / 免账号凭据**的方案。

> [!IMPORTANT]
> **当前免登录能力说明**
> - ✅ 支持：**公开单集链接**的免登录音频下载（例如 `https://www.xiaoyuzhoufm.com/episode/<episode_id>`）
> - ❌ 暂不支持免登录：播客整专辑批量下载、分页抓取更多集数、付费/私有内容、依赖登录态的字幕/元数据接口
> - ⚠️ 也就是说：**目前免登录模式仅保证“公开单集下载”这条最小链路**，其他功能暂时仍不保证可用

一个简单易用的小宇宙播客下载工具，支持批量下载播客专辑或单集。

## 主要功能

- **公开单集免登录下载** - 给定公开 `episode` 链接，优先通过网页公开数据提取音频直链并直接下载
- **Token 认证** - 保留 refresh_token 和 device_id 认证能力（但**不再推荐**使用）
- **播客下载** - 下载整个播客的所有集数（当前仍依赖鉴权能力）
- **单集下载** - 下载指定的单个播客集数
...
如果你只是下载**公开单集**，现在可以**直接免登录使用**：

```bash
xyz-dl https://www.xiaoyuzhoufm.com/episode/<episode_id>
```

只有在你确实要尝试旧的鉴权能力（例如整播客、受限内容、某些接口）时，才需要登录认证：
...
# 下载整个播客（注意：当前通常仍依赖登录态，不属于免登录支持范围）
xyz-dl 682c566cc7c5f17595635a2c

# 使用播客URL下载（注意：当前通常仍依赖登录态，不属于免登录支持范围）
...
**Q: 下载失败？**  
A: 先区分场景：如果是**公开单集**，优先直接用 `episode` 链接测试；如果是**整播客 / 批量 / 更多分页 / 付费内容**，当前通常仍需要登录态，而且并不稳定。
